...
- Governed-prompt token reduction: `~28%` vs raw memory (rough char/4 estimate, not a measured provider token count).

## Latest smoke test

Verified at `2026-07-03 00:31:38 JST`.
...
- Qwen errors: none.
- Prompt-token reduction: `~28%` (rough char/4 estimate of the governed vs raw prompt).
- UI screenshots:
