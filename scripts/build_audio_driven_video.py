...
SEGMENTS: tuple[Segment, ...] = (
    ...
    Segment(VIDEO_DIR / "erinys-caredog-problem.png", "That is the pain. Family health memory is messy, sensitive, and time-dependent. A stale note can create the wrong plan. A private identifier should not enter the model prompt.", pause=0.6),
    ...
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "00-ready.png", "Now let's view the live application running on Alibaba Cloud ECS. The public Docker demo is connected to Qwen Cloud with qwen3.7-plus."),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "01-initial-no-memory.png", "We ask the same clinic-visit question across three memory strategies. First, No Memory. Qwen is careful, but it lacks the appointment and transport context."),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "02-initial-raw-memory.png", "Second, Raw Memory. The answer is detailed, but unsafe. Old routines and private identifiers enter the raw prompt."),
    Segment(VIDEO_DIR / "erinys-caredog-private-id-proof.png", "Here is the proof cut. The raw prompt contains synthetic private IDs. ERINYS blocks those identifiers before Qwen sees the governed prompt.", pause=0.95),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "03-initial-decision-layer.png", "Now ERINYS classifies memory before Qwen sees the prompt. Each memory becomes selected, conflicted, demoted, or blocked."),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "04-initial-erinys-qwen.png", "Third, ERINYS plus Qwen. Only safe, current care context reaches the final answer, so the plan becomes specific without exposing private data."),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "05-save-memory.png", "Now we save a live care instruction: ask reception to arrange wheelchair assistance at the clinic entrance."),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "09-saved-erinys-qwen.png", "The benchmark runs again. The new memory can influence the answer, but only after ERINYS governance."),
    Segment(VIDEO_DIR / "build" / "app-tour-public-v8" / "10-saved-decisions.png", "The Memory Decisions panel is the proof. Current care is selected. Stale routines are demoted. Private identifiers are blocked."),
    Segment(VIDEO_DIR / "erinys-caredog-proof.png", "This is the outcome. Three synthetic private IDs appear in the raw prompt, and zero are sent in the governed prompt. The win is governance, not token trimming.", pause=1.0),
